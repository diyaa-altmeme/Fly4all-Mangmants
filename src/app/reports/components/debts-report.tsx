...
const getColumns = (): ColumnDef<DebtsReportEntry>[] => [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (type === 'both') return <Badge variant="secondary">عميل ومورد</Badge>;
            if (type === 'supplier') return <Badge variant="outline">مورد</Badge>;
            return <Badge>عميل</Badge>;
        }
    },
    {
        id: 'balanceUSD',
    ...
    },
    {
        ...
        cell: ({row}) => row.original.lastTransaction ? format(parseISO(row.original.lastTransaction), 'yyyy-MM-dd') : '-'
    },
    {
        id: 'actions',
                 ...
                 <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                            <Link href={`/reports/account-statement?accountId=${row.original.id}&currency=USD`}><FileText className="me-2 h-4 w-4"/>كشف حساب (USD)</Link>
                        </DropdownMenuItem>
                         <DropdownMenuItem asChild>
                            <Link href={`/reports/account-statement?accountId=${row.original.id}&currency=IQD`}><FileText className="me-2 h-4 w-4"/>كشف حساب (IQD)</Link>
                        </DropdownMenuItem>
...
export default function DebtsReport({ initialData }: DebtsReportProps) {
    ...
    const netBalanceUSD = summary.totalDebitUSD - summary.totalCreditUSD;
    const netBalanceIQD = summary.totalDebitIQD - summary.totalCreditIQD;
        ...
        <Card>
            <CardHeader>
                <div className="flex w-full flex-col items-start gap-4">
                     <div>
                        <CardTitle>تقرير الأرصدة النهائية</CardTitle>
                        <CardDescription>
                     ...
                     <div className="w-full flex flex-col sm:flex-row gap-2">
                        ...
                        <div className="flex items-center space-x-2 space-x-reverse">
                            ...
                            <Label htmlFor="hide-zero" className="whitespace-nowrap">إخفاء الأرصدة الصفرية</Label>
                        </div>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <StatCard title="إجمالي مطلوب لنا (دائنون لنا)" usd={summary.totalDebitUSD} iqd={summary.totalDebitIQD} className="border-green-500/50 bg-green-50 dark:bg-green-950/30" />
                    <StatCard title="إجمالي مطلوب منا (مدينون لنا)" usd={summary.totalCreditUSD} iqd={summary.totalCreditIQD} className="border-red-500/50 bg-red-50 dark:bg-red-950/30" />
