...
export default function EditVoucherPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [voucher, clientsResponse, users, boxes, settings] =
        await Promise.all([
          getVoucherById(id),
          getClients({ all: true }),
          getUsers(),
      ...
      const clients = allRelations.filter(
        (r) => r.relationType === 'client' || r.relationType === 'both'
      );
      const suppliers = allRelations.filter(
        (r) => r.relationType === 'supplier' || r.relationType === 'both'
      );
  ...
  if (error || !data) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          {error || 'فشل تحميل بيانات السند أو البيانات المرتبطة به.'}
        </AlertDescription>
      </Alert>
    );
  }
