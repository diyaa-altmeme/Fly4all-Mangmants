...
import AddPaymentsForm from './add-payments-form';
...
interface AddPaymentsDialogProps {
  exchangeId: string;
  exchanges: Exchange[];
...
export default function AddPaymentsDialog({ exchangeId, exchanges, onSuccess, isEditing = false, initialData, children }: AddPaymentsDialogProps) {
  ...
  const headerColor = themeSettings?.light?.accent ? `hsl(${themeSettings.light.accent})` : 'hsl(var(--accent))';
  
  const triggerSave = async () => {
      if (formRef.current) {
        setOpen(false);
        toast({ title: `جاري ${isEditing ? 'تحديث' : 'حفظ'} التسديدات...` });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {children || <Button variant="secondary"><PlusCircle className="me-2 h-4 w-4" />إضافة تسديد</Button>}
      </DialogTrigger>
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col p-0">
        ...
        >
          <DialogTitle className="text-white">{isEditing ? `تعديل دفعة التسديد: ${initialData?.invoiceNumber}` : 'إدخال تسديدات جديدة للبورصة'}</DialogTitle>
        </DialogHeader>
        <div className="flex-grow overflow-y-auto px-2 sm:px-6 py-2">
         ...
         <DialogFooter className="p-2 sm:p-4 border-t flex-col sm:flex-row justify-between items-center bg-background">
            ...
            <Button onClick={triggerSave} disabled={isSaving || stagedEntriesCount === 0} className="w-full sm:w-auto" size="lg">
                ...
                {isEditing ? 'حفظ كل التعديلات' : `حفظ (${stagedEntriesCount}) تسديدات`}
            </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
