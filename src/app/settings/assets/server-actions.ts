...
export async function addSiteAsset(assetData: SiteAsset): Promise<{ success: boolean, error?: string }> {
    ...
    try {
        await db.collection('site_assets').doc(assetData.id).set(assetData);
...
export async function permanentDeleteAsset(asset: SiteAsset): Promise<{ success: boolean, error?: string }> {
    ...
    try {
        ...
        await db.collection('site_assets').doc(asset.id).delete();
...
export async function assignAsset(assetId: string, fullPath: string, assignmentPath: string | null): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        await db.runTransaction(async (transaction) => {
            const assetQuery = await db.collection('site_assets').where('fullPath', '==', fullPath).limit(1).get();
            ...
            if (assignmentPath) {
                const oldAssignmentQuery = await transaction.get(db.collection('site_assets').where('assignment', '==', assignmentPath));
