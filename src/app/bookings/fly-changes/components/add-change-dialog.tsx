...
import type { FlyChangeEntry, Client, Supplier } from '@/lib/types';
import { Loader2, Calendar as CalendarIcon, Save } from 'lucide-react';
import { useForm } from 'react-hook-form';
import { z } from 'zod';
import { zodResolver } from '@hookform/resolvers/zod';
import { saveFlyChange } from '../actions';
...
export default function AddChangeForm({ clients, suppliers, onSuccess }: AddChangeFormProps) {
  ...
  const handleSubmit = async (data: FormValues) => {
    const payload: Omit<FlyChangeEntry, 'id' | 'invoiceNumber'> = {
      ...data,
    ...
    const result = await saveFlyChange(payload);

    if (result.success) {
      toast({ title: 'تمت إضافة التغيير بنجاح' });
      onSuccess();
      form.reset();
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <div className="flex justify-end">
          <Button type="submit" disabled={isSubmitting}>
            ...
            <Save className="me-2 h-4 w-4" /> حفظ التغيير
          </Button>
        </div>
      </form>
    </Form>
