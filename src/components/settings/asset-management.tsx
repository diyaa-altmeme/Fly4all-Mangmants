...
const AssetSlot = ({ label, description, assetUrl, onAssign, assignmentPath }: {
    label: string;
    description: string;
    assetUrl?: string | null;
    onAssign: (asset: SiteAsset) => void;
    assignmentPath: string;
}) => {
    const { toast } = useToast();

    const handleUnassign = async () => {
         const result = await assignAsset('', '', null);
         if (result.success) {
    ...
    return (
        <Card className="overflow-hidden">
            ...
            <CardFooter className="p-3 bg-muted/30 border-t flex justify-end gap-2">
                 <Button variant="ghost" size="sm" onClick={handleUnassign} disabled={!assetUrl}>إلغاء التعيين</Button>
                {/* This will be replaced with a dialog to select from existing assets */}
...
export default function AssetManagementSettings({ settings, onSettingsChanged }: AssetManagementProps) {
    ...
    const handleAssign = (asset: SiteAsset, assignmentPath: string) => {
        assignAsset(asset.id, asset.fullPath, assignmentPath).then(result => {
             if (result.success) {
                onSettingsChanged();
             }
        });
                     ...
                     {assetSlots.map(slot => {
                        ...
                        return (
                            <AssetSlot 
                                ...
                                assetUrl={assetUrl}
                                onAssign={(asset) => handleAssign(asset, slot.id)}
