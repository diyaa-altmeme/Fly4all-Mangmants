  ...
  const menuConfig = useMemo(() => {
    const allMenus = [
        ...
        },
        {
            ...
            items: getVisibleItems([
              { href: "/reports/account-statement", label: "كشف الحساب", icon: FileText, permission: 'reports:account_statement' },
    ...
    return allMenus.filter(menu => menu.id === 'main' || menu.items.length > 0);
                              ...
                              <AccordionContent className="pl-6 border-l-2 border-primary/50 ml-4">
                                  <div className="flex flex-col gap-1 mt-2">
                                      {menu.items.map((item: any, idx: number) => {
                                          if (item.component) return React.cloneElement(item.component, { key: `comp-${idx}` });
                                          if (item.isSeparator) return <Separator key={`sep-${idx}`} className="my-2" />;
                                          if (item.isLabel) return <p key={`label-${idx}`} className="px-3 py-1.5 text-xs font-semibold text-muted-foreground">{item.label}</p>;
